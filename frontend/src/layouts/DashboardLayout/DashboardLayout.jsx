        ...
        <div className={styles.bellDropdown}>
          ...
          <div className={styles.bellList}>
            {notifications?.length ? notifications.slice(0, 10).map((n) => (
              <div key={n.id} className={`${styles.bellItem} ${n.read ? '' : styles.bellItemUnread}`}>
                <strong>{n.title}</strong>
                <span>{n.message}</span>
                <small>{new Date(n.time).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })}</small>
              </div>
            )) : <p className={styles.bellEmpty}>Sin notificaciones</p>}
          </div>
        </div>
      ) : null}
    </div>
